            ...
            try
            {
                ...
                if (sigType == SignatureType.FES)
                {
                    if (fieldsToSign.Count > 0)
                    {
                        ...
                        var bioKey = BiometricCertManager.GetEncryptionPublicKey();
                ...
                // Persist the auto-generated biometric key next to the saved document,
                // named after it: "<SavedName>_bioCert.pfx" — but only when biometric data was
                // actually captured (no ink → no biometric data → no PFX needed). No-op if a
                // BiometricCertPath is configured (that certificate is reused for every document).
                if (fieldsToSign.Any(f => f.BioPoints.Count > 0))
                    BiometricCertManager.SaveAutoGeneratedPfx(
                        Path.GetDirectoryName(finalPath)!, Path.GetFileNameWithoutExtension(finalPath));
